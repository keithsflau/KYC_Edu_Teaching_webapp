...
const TILE_SRC = (function () {
  ...
  // Proxy only when using dedicated serve.js (port 5051). Plain http.server / GitHub Pages need upstream.
  const useProxy =
    (location.hostname === "localhost" || location.hostname === "127.0.0.1") &&
    location.port === "5051";
  return useProxy ? proxy : upstream;
})();
...
/** Hermite smoothstep for JS water-weight falloff (GLSL smoothstep is shader-only). */
